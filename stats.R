...
MdDaFILF<-subset_samples(macDaFp,Sample_Type%in%c("ILF")) # keep only ILF samples
MdDaFInt<-subset_samples(macDaFp,Sample_Type%in%c("Intestinum")) # keep only Intestinum samples


# by ILF
coreILF<-subset_samples(coreMdHv,Sample_Type=="ILF") # Examine only ILF data
...
permutest(beta)
# by Intestinum
coreInt<-subset_samples(coreMdHv,Sample_Type=="Intestinum") # Examine only Intestinum data
...
# by Bladder
coreBlad<-subset_samples(coreMdHv,Sample_Type=="Bladder") # Examine only Bladder data
...
# by Hirudo
coreHv<-subset_samples(coreMdHv,Taxonomic_ID=="Hverbana") # Examine only Hverbana data
...
# by Macrobdella
coreMd<-subset_samples(coreMdHv,Taxonomic_ID=="Mdecora") # Examine only Mdecora data
...
# Hirudo GI
coreHvGI<-subset_samples(coreHv,Sample_Type%in%c("ILF","Intestinum")) # Examine only Hverbana ILF and Intestinum data
...
sig.coreHvGI
# Macrobdella GI
coreMdGI<-subset_samples(coreMd,Sample_Type%in%c("ILF","Intestinum")) # Examine only Mdecora ILF and Intestinum data
...
list.Da1F<-levels(factor(sample_data(MdDaFILF)$Da1F))
...
for(day1 in c(list.Da1F)){
  ...
  for(day in c(list.Da1F)){ 
    print(day)
    if(day==day1){next}
    MdLoopILF<-subset_samples(MdDaFILF,Da1F%in%c(day1,day)) # Examine only ILF data
    ...
    perm.MdLoopILF<-adonis(bray.MdLoopILF ~ Da1F, data = df.sampleMdLoopILF, method = "bray") # Adonis test
    sig.MdLoopILF<-as.data.frame(perm.MdLoopILF$aov.tab)["Da1F", "Pr(>F)"]
    print(sig.MdLoopILF)
...
list.Da1F<-levels(factor(sample_data(MdDaFInt)$Da1F))
...
for(day1 in c(list.Da1F)){
  ...
  for(day in c(list.Da1F)){ 
    print(day)
    if(day==day1){next}
    MdLoopInt<-subset_samples(MdDaFInt,Da1F%in%c(day1,day)) # Examine only Int data
    ...
    perm.MdLoopInt<-adonis(bray.MdLoopInt ~ Da1F, data = df.sampleMdLoopInt, method = "bray") # Adonis test
    sig.MdLoopInt<-as.data.frame(perm.MdLoopInt$aov.tab)["Da1F", "Pr(>F)"]
    print(sig.MdLoopInt)
...
phy.Md<-subset_samples(phyBasC,Taxonomic_ID=="Mdecora") # Keep only Macrobdella samples
list.Type<-levels(factor(sample_data(phy.Md)$Sample_Type))
...
for(t1 in c(list.Type)){
  ...
  for(type in c(list.Type)){ 
    print(type)
    if(type==t1){next}
    MdLoopType<-subset_samples(phy.Md,Sample_Type%in%c(t1,type)) # Examine only Type data
    ...
    perm.MdLoopType<-adonis(bray.MdLoopType ~ Sample_Type, data = df.sampleMdLoopType, method = "bray") # Adonis test
    sig.MdLoopType<-as.data.frame(perm.MdLoopType$aov.tab)["Sample_Type", "Pr(>F)"]
...
phy.Hv<-subset_samples(phyBasC,Taxonomic_ID=="Hverbana") # Keep only Macrobdella samples
list.Type<-levels(factor(sample_data(phy.Hv)$Sample_Type))
...
for(t1 in c(list.Type)){
  ...
  for(type in c(list.Type)){ 
    print(type)
    if(type==t1){next}
    HvLoopType<-subset_samples(phy.Hv,Sample_Type%in%c(t1,type)) # Examine only Type data
    ...
    perm.HvLoopType<-adonis(bray.HvLoopType ~ Sample_Type, data = df.sampleHvLoopType, method = "bray") # Adonis test
    sig.HvLoopType<-as.data.frame(perm.HvLoopType$aov.tab)["Sample_Type", "Pr(>F)"]
